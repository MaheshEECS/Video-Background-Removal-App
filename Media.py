...
def remove_background(video_path, output_path, bg_type, bg_color=(0, 255, 0), bg_image=None, transparent=False, mask_threshold=0.5):
    cap = None
    out = None
    ...
    try:
        ...
        with mp_selfie_segmentation.SelfieSegmentation(model_selection=1) as selfie_segmentation:
            while cap.isOpened():
                ...
                mask = results.segmentation_mask > mask_threshold

                # Morphological operation to smooth the mask
                kernel = np.ones((5, 5), np.uint8)
                mask = cv2.dilate(mask.astype(np.uint8), kernel, iterations=1)

                # Create the output frame based on the selected background type
                if transparent:
                    output_frame = np.zeros_like(frame)
                    output_frame[mask] = frame[mask]
                elif bg_type == "Image":
                    # Ensure background image matches the frame size
...
if uploaded_video:
    try:
        ...
        # Adjust segmentation mask threshold
        st.write("Adjust segmentation mask threshold:")
        mask_threshold = st.slider("Threshold", min_value=0.0, max_value=1.0, value=0.5, step=0.05)
